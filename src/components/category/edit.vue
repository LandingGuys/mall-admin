...
<script>
import {  getCategoryList, categoryUpdate , categoryDetail} from '@/api/index.js'
export default {
    data(){
        return{
            form: {
    ...
    methods:{
      ...
      async init(){
        if (this.$route.query.id == null) {
         ...
         const res =await categoryDetail(params.params)
         if(res.status!==0){
             this.$message.error(res.msg)
         }
         console.log(res)
         ...
         this.form.sortOrder =res.data.sortOrder
          if(this.status === 1){
                this.form.status = "上架"
            }else{
                this.form.status = "下架"
